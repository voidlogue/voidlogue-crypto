...
describe("VoidShield.roomId", () => {
  it("returns a 64-character hex string", async () => {
    const id = await VoidShield.roomId("alice@example.com", "bob@example.com", "iron-falcon");
    expect(id).toHaveLength(64);
    expect(id).toMatch(/^[0-9a-f]+$/);
  });

  it("is commutative — order of emails does not matter", async () => {
    const id1 = await VoidShield.roomId("alice@example.com", "bob@example.com", "iron-falcon");
    const id2 = await VoidShield.roomId("bob@example.com", "alice@example.com", "iron-falcon");
    expect(id1).toBe(id2);
  });

  it("is deterministic — same inputs always produce same room hash", async () => {
    const id1 = await VoidShield.roomId("alice@example.com", "bob@example.com", "iron-falcon");
    const id2 = await VoidShield.roomId("alice@example.com", "bob@example.com", "iron-falcon");
    expect(id1).toBe(id2);
  });

  it("different codenames produce different room hashes", async () => {
    const id1 = await VoidShield.roomId("alice@example.com", "bob@example.com", "iron-falcon");
    const id2 = await VoidShield.roomId("alice@example.com", "bob@example.com", "silver-moon");
    expect(id1).not.toBe(id2);
  });

  it("different email pairs produce different room hashes", async () => {
    const id1 = await VoidShield.roomId("alice@example.com", "bob@example.com", "iron-falcon");
    const id2 = await VoidShield.roomId("charlie@example.com", "bob@example.com", "iron-falcon");
    expect(id1).not.toBe(id2);
  });
...
describe("VoidShield.validateCodename", () => {
  it("accepts valid codenames", () => {
    ...
    expect(VoidShield.validateCodename("correct-horse-battery-staple").valid).toBe(true);
...
describe("VoidShield encrypt/decrypt round-trip", () => {
  it("encrypts and decrypts a message correctly", async () => {
    const roomHash = await VoidShield.roomId("a@b.com", "c@d.com", "test-codename");
    const key      = await VoidShield.deriveKey("test-codename", roomHash);
    const { ciphertextB64, ivB64 } = await VoidShield.encrypt("hello world", key);
  ...
  it("decryption fails with wrong key (authentication tag mismatch)", async () => {
    ...
    await expect(VoidShield.decrypt(ciphertextB64, ivB64, key2)).rejects.toThrow();
  });
  ...
  it("decryption fails with tampered ciphertext", async () => {
    ...
    const { ciphertextB64, ivB64 } = await VoidShield.encrypt("secret message", key);
...
describe("VoidShield.deriveRevelationKey", () => {
  it("returns a non-null CryptoKey", async () => {
    const key = await VoidShield.deriveRevelationKey(
      "sender@example.com", "recipient@example.com", ["Alice", "1990-01-01"]
    );
  ...
  it("is commutative on email order", async () => {
    const k1 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", ["field"]);
    const k2 = await VoidShield.deriveRevelationKey("r@b.com", "s@a.com", ["field"]);
  ...
  it("different field values produce different keys", async () => {
    const k1 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", ["Alice"]);
    const k2 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", ["Bob"]);
    const { ciphertextB64, ivB64 } = await VoidShield.encrypt("test", k1);
    await expect(VoidShield.decrypt(ciphertextB64, ivB64, k2)).rejects.toThrow();
  });

  it("normalises field values (case + whitespace insensitive)", async () => {
    const k1 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", ["alice"]);
    const k2 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", [" Alice "]);
    const { ciphertextB64, ivB64 } = await VoidShield.encrypt("test", k1);
    const result = await VoidShield.decrypt(ciphertextB64, ivB64, k2);
    expect(result).toBe("test");
  });
...
describe("VoidShield.deriveRevelationKeyFromHashes", () => {
  it("produces same key as deriveRevelationKey given same inputs", async () => {
    ...
    const k2 = await VoidShield.deriveRevelationKeyFromHashes(sHash, rHash, fields);

    const { ciphertextB64, ivB64 } = await VoidShield.encrypt("revelation content", k1);
    const result = await VoidShield.decrypt(ciphertextB64, ivB64, k2);
...
describe("VoidShield media encryption", () => {
  ...
  it("decryptMediaStream recovers original file bytes", async () => {
    ...
    const file   = new File([original], "test.bin", { type: "application/octet-stream" });
    ...
    const recovered = new Uint8Array(buffers.reduce((acc, b) => acc + b.length, 0));
    let offset = 0;
    for (const b of buffers) { recovered.set(b, offset); offset += b.length; }
...
describe("generateCodename", () => {
  ...
  it("all words come from the EFF wordlist", () => {
    ...
    for (let i = 0; i < 20; i++) {
      generateCodename(4).split("-").forEach(w => {
...
describe("Security invariants", () => {
  it("room hash does not reveal email A", async () => {
    const id = await VoidShield.roomId("alice@secret.com", "bob@example.com", "codename");
    expect(id).not.toContain("alice");
    expect(id).not.toContain("secret");
  });

  it("room hash does not reveal email B", async () => {
    const id = await VoidShield.roomId("alice@example.com", "bob@secret.com", "codename");
    expect(id).not.toContain("bob");
    expect(id).not.toContain("secret");
  });
  ...
  it("two Revelation keys derived from same inputs are functionally identical", async () => {
    const k1 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", ["fieldvalue"]);
    const k2 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", ["fieldvalue"]);
    const { ciphertextB64, ivB64 } = await VoidShield.encrypt("revelation", k1);
    expect(await VoidShield.decrypt(ciphertextB64, ivB64, k2)).toBe("revelation");
  });

  it("wrong field value cannot decrypt revelation", async () => {
    const k1 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", ["correct-value"]);
    const k2 = await VoidShield.deriveRevelationKey("s@a.com", "r@b.com", ["wrong-value"]);
